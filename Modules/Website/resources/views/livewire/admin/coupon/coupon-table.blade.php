<div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Quản lý Mã giảm giá</h1>
            <p class="mt-1 text-sm text-gray-500">Chiến dịch marketing và khuyến mãi.</p>
        </div>
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ route('admin.coupons.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-indigo-600 hover:bg-indigo-700 transition">
    ...
    <div class="mb-6">
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden transition-all duration-300 relative">
            ...
            <div wire:loading.flex wire:target="search, deleteSelected, toggleStatus" class="absolute inset-0 bg-white/60 z-20 items-center justify-center backdrop-blur-[1px]">
                ...
                <div class="p-3 bg-indigo-50 flex items-center justify-between animate-fade-in">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-sm font-semibold text-indigo-900">
                            Đã chọn <span class="font-bold text-indigo-700 text-base mx-1">{{ count($selected) }}</span> mã
                        </span>
                    </div>
                    <button wire:click="deleteSelected" wire:confirm="Xóa các mã giảm giá này?" class="flex items-center px-4 py-2 bg-white border border-red-200 text-red-600 rounded-lg text-sm font-bold shadow-sm hover:bg-red-50 transition">
            ...
            @else
                <div class="p-2 flex gap-2">
                    <div class="relative flex-1">
                        ...
                        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Tìm theo mã code..." class="block w-full pl-10 pr-3 py-2 border-transparent bg-transparent focus:border-transparent focus:ring-0 text-gray-900 placeholder-gray-400 sm:text-sm h-10 transition">
                    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50/50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Mã Code</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Giá trị giảm</th>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Hiệu lực</th>
                        <th class="relative px-6 py-3 text-right"><span class="sr-only">Actions</span></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200 bg-white">
                    @forelse($coupons as $coupon)
                        <tr class="hover:bg-gray-50 transition duration-150 {{ in_array($coupon->id, $selected) ? 'bg-indigo-50/40' : '' }}">
                            
                            <td class="px-4 py-4 text-center">
                                <input type="checkbox" value="{{ $coupon->id }}" wire:model.live="selected" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600">
                            </td>
                            
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="h-10 w-10 flex-shrink-0 bg-indigo-50 text-indigo-600 rounded-lg flex items-center justify-center border border-indigo-100 font-bold">
                                        %
                                    </div>
                                    <div class="ml-3">
                                        <div class="text-sm font-bold text-gray-900 font-mono tracking-wide">{{ $coupon->code }}</div>
                                        <div class="text-xs text-gray-500 line-clamp-1" title="{{ $coupon->description }}">{{ $coupon->description }}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @endif
                                <div class="text-xs text-gray-400 mt-1">Đơn tối thiểu: {{ number_format($coupon->min_order_value) }} ₫</div>
                            </td>

                            <td class="px-6 py-4 text-center w-48">
                                <div class="flex items-center justify-between text-xs text-gray-500 mb-1">
                                    <span class="font-medium">{{ $coupon->usage_count }} dùng</span>
                                    <span>{{ $coupon->usage_limit ? '/ '.$coupon->usage_limit : '∞' }}</span>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-1.5 overflow-hidden">
                                    @php
                                        $percent = $coupon->usage_limit > 0 ? ($coupon->usage_count / $coupon->usage_limit) * 100 : 0;
                                        $colorClass = $percent > 90 ? 'bg-red-500' : 'bg-indigo-600';
                                    @endphp
                            ...
                            <td class="px-6 py-4 text-center">
                                <button wire:click="toggleStatus({{ $coupon->id }})" class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none {{ $coupon->is_active ? 'bg-green-500' : 'bg-gray-200' }}">
                                ...
                                @if(!$coupon->is_valid)
                                    <div class="text-[10px] text-red-500 mt-1 font-bold">Hết hạn</div>
                                @endif
                            </td>

                            <td class="px-6 py-4 text-xs text-gray-500 whitespace-nowrap">
                                <div><span class="font-medium">BĐ:</span> {{ $coupon->starts_at ? $coupon->starts_at->format('d/m/Y') : 'Ngay lập tức' }}</div>
                                <div><span class="font-medium">KT:</span> {{ $coupon->expires_at ? $coupon->expires_at->format('d/m/Y') : 'Vô thời hạn' }}</div>
                            </td>

                            <td class="px-6 py-4 text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-3">
                                    <a href="{{ route('admin.coupons.edit', $coupon->id) }}" class="text-gray-400 hover:text-indigo-600 transition" title="Sửa">
                                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <button wire:confirm="Xóa mã này?" wire:click="delete({{ $coupon->id }})" class="text-gray-400 hover:text-red-600 transition" title="Xóa">
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                                <div class="flex flex-col items-center justify-center">
                                    <svg class="h-12 w-12 text-gray-300 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z"/></svg>
                                    <span>Chưa có mã giảm giá nào. Hãy import hoặc tạo mới!</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <div class="bg-gray-50 border-t border-gray-200 px-4 py-3 sm:px-6">{{ $coupons->links() }}</div>
    </div>
